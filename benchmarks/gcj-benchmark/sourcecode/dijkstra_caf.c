...
void do_test(int t)
{
	...
	q_type pattern_tail_prod[10001];
	...
	pattern_tail_prod[l] = Q_1;
	for (i = l - 1; i >= 0; i--)
		pattern_tail_prod[i] = q_mult[pattern[i]][pattern_tail_prod[i + 1]];

	assert(pattern_prod == pattern_tail_prod[0]);
...
/*						printf("\tFound j (%d)\n", m - n);*/
						...
						c_prod = q_mult[c_prod][pattern_tail_prod[m + 1]];
...
/*							printf("\tFound j (%d + P * %d + %d)\n", l - n - 1, b, m);*/
							...
							c_prod = q_mult[c_prod][pattern_tail_prod[m]];
